# Coupa Request App - Functional Specification

## Overview
Handles supplier onboarding requests originating from Coupa. Focuses on banking information and tax compliance.

## Navigation

### Sections
- [General Information](sections/general-information.md)
- [Payment Information](sections/payment-information.md)
- [Addresses](sections/addresses.md)
- [Bank Accounts](sections/bank-accounts.md)
- [Vat Ids](sections/vat-ids.md)
- [Identifications](sections/identifications.md)

### Workflows
- [Create Request](workflows/create-request.md)
- [Change Request](workflows/change-request.md)

### Validation
- [Field Validations](validation/field-validations.md)
- [Section Validations](validation/section-validations.md)

### API
- [Endpoints](api/endpoints.md)
- [Examples](api/examples.md)

## Key Characteristics
| Characteristic | Value |
|:---------------|:------|
| Entity Type | Supplier |
| Source System | Coupa |

← Back to [Main Documentation](../../README.md)
