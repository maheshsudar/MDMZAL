...
class NotificationService {
  ...
  constructor() {
    console.log('DEBUG: NotificationService constructor start');
    this.webhookQueue = [];
    this.emailQueue = [];
  ...
  buildWebhookPayload(request, event, context) {
    const payload = {
      ...
      notificationId: context.notificationId || `NOTIF-${Date.now()}`,  // For acknowledgement tracking
      ...
      sapBpNumber: request.sapBpNumber || null,
    };
  ...
  async getWebhookUrls(sourceSystem) {
    try {
      ...
      // Default webhook URLs for testing (using satellite mock service)
      const defaultUrls = {
        'Coupa': ['http://localhost:4004/satellite-mock/coupa/webhook/mdm-approval'],
        'Salesforce': ['http://localhost:4004/satellite-mock/salesforce/webhook/mdm-approval'],
        'PI': ['http://localhost:4004/satellite-mock/pi/webhook/mdm-approval']
      };
  ...
  /**
   * Send satellite system acknowledgement email
   * Notifies a team that their BP is now being shared with another system
   * @param {String} targetSystem - Target system ('Coupa', 'Salesforce', 'PI')
   * @param {Object} request - Business partner request object
   */
  async sendSatelliteAcknowledgementEmail(targetSystem, request) {
    ...
    const distributionList = distributionLists[targetSystem] || 'mdm-team@company.com';

    const subject = `[Action Required] BP ${request.requestNumber} - Acknowledgement Needed`;

    const emailBody = `
Dear ${targetSystem} Team,

A business partner that you currently manage has been updated by another system and is now being shared.

Request Details:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Request Number:   ${request.requestNumber}
  Partner Name:     ${request.name1 || request.partnerName || 'N/A'}
  SAP BP Number:    ${request.sapBpNumber || 'Not yet created'}
  Updated By:       ${request.sourceSystem}
  Status:           Approved
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

ACTION REQUIRED:
Please review and acknowledge this change in the Satellite System Acknowledgement app.

📋 Access the app:
   http://localhost:4004/satellite-ack-${targetSystem.toLowerCase()}/webapp/index.html

Why this matters:
This business partner is now managed across multiple systems. Please review the changes to ensure alignment with your system's data and processes.
...
MDM System Team
    ...
    try {
      console.log(`📧 Sending satellite acknowledgement email to ${targetSystem} team`);
      console.log(`   To: ${distributionList}`);
      console.log(`   Subject: ${subject}`);
      console.log(`   Request: ${request.requestNumber}`);

      // In production, this would use an actual email service (SMTP, SendGrid, etc.)
      // For now, we'll just log it
      console.log('   ✅ Email sent successfully (mock)');

      // Log to email queue for processing
      this.emailQueue.push({
        id: `ack-${request.requestNumber}-${targetSystem}`,
        to: [distributionList],
        subject: subject,
        body: emailBody,
        requestNumber: request.requestNumber,
      ...
      return {
        success: true,
        sentTo: distributionList,
        targetSystem: targetSystem
      };

    } catch (error) {
      console.error(`❌ Failed to send satellite acknowledgement email:`, error);
      throw error;
    }
  }
}
