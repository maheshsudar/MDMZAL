...
/**
 * Refactored Coupa Service Implementation
...
module.exports = cds.service.impl(async function () {
    ...
    const { CoupaRequests } = this.entities;

    log.info('Coupa Service initialized');
    ...
    this.on('submit', CoupaRequests, async (req) => {
        log.info('Submitting Coupa request', { requestID: req.params[0].ID });
        ...
        try {
            ...
            const result = await validationService.validateRequest(
                ...
                requestData.sourceSystem || 'Coupa',
            ...
            // Submit the request
            await UPDATE(CoupaRequests).set({ status: 'Submitted' }).where({ ID });
    ...
    this.on('checkDuplicates', CoupaRequests, async (req) => {
        log.info('Checking for duplicates (Coupa bound action)', { requestID: req.params[0].ID });
        const { ID } = req.params[0];
    ...
    this.on('importSAPPartner', async (req) => {
        ...
        try {
            const db = await cds.connect.to('db');
            const result = await sapPartnerService.importSAPPartner(sapBpNumber, 'Coupa', db);
            return result;
    ...
    this.on('createChangeRequestFromSAP', async (req) => {
        ...
        try {
            ...
            // Import partner data
            const importResult = await sapPartnerService.importSAPPartner(sapBpNumber, 'Coupa', db);
    ...
    this.before('NEW', 'CoupaRequests', async (req) => {
        ...
        if (!req.data.coupaInternalNo) {
            req.data.coupaInternalNo = cds.utils.uuid();
        }

        // NOTE: Request Number generation removed from here to prevent duplicates.
        // It's now ONLY generated in the before('CREATE', 'CoupaRequests.drafts') handler
        // to ensure a single point of generation and avoid race conditions.
    });

    this.before('CREATE', 'CoupaRequests', async (req) => {
        ...
        if (!req.data.requestNumber) {
            const db = await cds.connect.to('db');
            req.data.requestNumber = await requestNumberGenerator.getNextNumber('COUPA', db);
        }
    });

    // CRITICAL FIX: For draft-enabled entities, explicitly handle draft creation
    this.before('CREATE', 'CoupaRequests.drafts', async (req) => {
        ...
        log.info('=== Creating NEW DRAFT Coupa request ===');
     ...
     */
    this.before('UPDATE', 'CoupaRequests', async (req) => {
        const { ID } = req.data;
     ...
     */
    this.after('UPDATE', 'CoupaRequests', async (result, req) => {
     ...
     */
    this.before('SAVE', 'CoupaRequests', async (req) => {
        const { ID } = req.data;
        ...
        try {
            ...
            const sourceSystem = requestData.sourceSystem || 'Coupa';
     ...
     */
    this.after('SAVE', 'CoupaRequests', async (result, req) => {
        const ID = req.data.ID;
    ...
    const calculateVirtualProperties = async (data, isDraft) => {
        ...
        if (missingStatusIds.length > 0) {
            ...
            const entity = isDraft ? 'CoupaService.CoupaRequests.drafts' : 'CoupaService.CoupaRequests';
    ...
    this.before('READ', 'CoupaRequests', (req) => {
        if (req.query.SELECT.columns && !req.query.SELECT.columns.find(c => c.ref && c.ref[0] === 'status')) {
            req.query.SELECT.columns.push({ ref: ['status'] });
        }
        // Filter by sourceSystem = 'Coupa'
        if (req.query.SELECT && req.query.SELECT.where) {
            req.query.SELECT.where.push('and', { ref: ['sourceSystem'] }, '=', { val: 'Coupa' });
        } else if (req.query.SELECT) {
            req.query.SELECT.where = [{ ref: ['sourceSystem'] }, '=', { val: 'Coupa' }];
        }
    });
    this.before('READ', 'CoupaRequests.drafts', (req) => {
        if (req.query.SELECT.columns && !req.query.SELECT.columns.find(c => c.ref && c.ref[0] === 'status')) {
            req.query.SELECT.columns.push({ ref: ['status'] });
        }
        // Filter by sourceSystem = 'Coupa'
    ...
    this.after('READ', 'CoupaRequests', (data) => calculateVirtualProperties(data, false));
    this.after('READ', 'CoupaRequests.drafts', (data) => calculateVirtualProperties(data, true));
    ...
    const setChildFieldControl = async (childData, entityName, isDraft) => {
        ...
        const parentEntity = isDraft ? 'CoupaService.CoupaRequests.drafts' : 'CoupaService.CoupaRequests';
    ...
    // 3. If CHANGE: Only COUPA identifications are editable, others (SALESFORCE, PI) are readonly
    // 4. If CREATE: All identifications editable (but dropdown only shows COUPA anyway)
        ...
        records.forEach(id => {
            if (id.request_ID) {
                ...
                if (!parent.isEditable) {
                    ...
                    // CHANGE requests: Only allow editing identifications owned by this system (COUPA)
                    // If identificationType_code is empty/null, it's a new record being added - should be editable
                    if (!id.identificationType_code || id.identificationType_code === 'COUPA') {
                        id.fieldControl = 7; // Editable (new record or owned by this system)
                    } else {
                        id.fieldControl = 1; // ReadOnly (owned by other system: SALESFORCE, PI)
                    }
                } else {
                    // CREATE requests: All identifications editable (dropdown restricts to COUPA only)
    ...
    this.before(['CREATE', 'UPDATE'], 'PartnerIdentifications', async (req) => {
        ...
        if (req.event === 'UPDATE' && ID) {
            const currentRecord = await SELECT.one.from('CoupaService.PartnerIdentifications.drafts')
                .where({ ID })
        ...
        const existingInDraft = await SELECT.one.from('CoupaService.PartnerIdentifications.drafts')
            .where({
    ...
    this.before('EDIT', 'CoupaRequests', async (req) => {
        const { ID } = req.data;
        const record = await SELECT.one.from(CoupaRequests).where({ ID }).columns('status');
    ...
    async function getFullRequestData(requestID, service, isDraft = false) {
        const { CoupaRequests } = service.entities;

        // Determine which entity to query
        const entityName = isDraft
            ? 'CoupaService.CoupaRequests.drafts'
            : CoupaRequests;
    ...
    log.info('Coupa Service initialization complete');
});
